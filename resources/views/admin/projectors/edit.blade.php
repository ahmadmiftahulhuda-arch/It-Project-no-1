...
<head>
    ...
    <title>Admin TI - Edit Proyektor</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        /* SAMA PERSIS DENGAN CSS DI CREATE DI ATAS */
        :root {
            --primary: #3b5998;
            --secondary: #6d84b4;
            --success: #4caf50;
...
<!-- Sidebar -->
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="/admin/projectors" class="menu-item active">
    ...
    <div class="main-content">
        ...
        <div class="page-title">
            <div>
                <h1>Edit Proyektor</h1>
                <p>Perbarui data proyektor</p>
            </div>
            <div>
            ...
            <form action="{{ route('projectors.update', $projector->id) }}" method="POST">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    <div>
                        ...
                        <input type="text" class="form-control" id="kode_proyektor" name="kode_proyektor" 
                               value="{{ old('kode_proyektor', $projector->kode_proyektor) }}" placeholder="PROJ-001" required>
                    ...
                    <div>
                        ...
                        <input type="text" class="form-control" id="merk" name="merk" 
                               value="{{ old('merk', $projector->merk) }}" placeholder="Epson" required>
                        @error('merk')
                            <div style="color: #dc3545; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" class="form-control" id="model" name="model" 
                               value="{{ old('model', $projector->model) }}" placeholder="EB-X06" required>
                        @error('model')
                            <div style="color: #dc3545; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select class="form-control" id="status" name="status" required>
                            ...
                            <option value="tersedia" {{ old('status', $projector->status) == 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                            <option value="dipinjam" {{ old('status', $projector->status) == 'dipinjam' ? 'selected' : '' }}>Dipinjam</option>
                            <option value="rusak" {{ old('status', $projector->status) == 'rusak' ? 'selected' : '' }}>Rusak</option>
                        </select>
                        @error('status')
                            <div style="color: #dc3545; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
                        @enderror
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea class="form-control" id="keterangan" name="keterangan" 
                              rows="3" placeholder="Keterangan tambahan...">{{ old('keterangan', $projector->keterangan) }}</textarea>
                    @error('keterangan')
                        <div style="color: #dc3545; font-size: 0.875rem; margin-top: 5px;">{{ $message }}</div>
                    @enderror
                </div>
