...
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-title">
            <div>
                <h1>Edit Ruangan</h1>
                <p>Perbarui informasi ruangan {{ $ruangan->nama_ruangan }}</p>
            </div>
            ...
            <form action="{{ route('admin.ruangan.update', $ruangan->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" id="kode_ruangan" name="kode_ruangan" class="form-control @error('kode_ruangan') is-invalid @enderror" 
                                   value="{{ old('kode_ruangan', $ruangan->kode_ruangan) }}" required readonly>
                            @error('kode_ruangan')
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" id="nama_ruangan" name="nama_ruangan" class="form-control @error('nama_ruangan') is-invalid @enderror" 
                                   value="{{ old('nama_ruangan', $ruangan->nama_ruangan) }}" required placeholder="Masukkan nama ruangan">
                            @error('nama_ruangan')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" id="kapasitas" name="kapasitas" class="form-control @error('kapasitas') is-invalid @enderror" 
                                   value="{{ old('kapasitas', $ruangan->kapasitas) }}" min="1" required placeholder="Jumlah orang">
                            @error('kapasitas')
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select id="status" name="status" class="form-control @error('status') is-invalid @enderror" required>
                                ...
                                <option value="Sedang Digunakan" {{ old('status', $ruangan->status) == 'Sedang Digunakan' ? 'selected' : '' }}>Sedang Digunakan</option>
                            ...
                            <div class="mt-2">
                                <small>Preview Status: 
                                    <span class="status-preview 
                                        @if(old('status', $ruangan->status) == 'Tersedia') status-tersedia
                                        @elseif(old('status', $ruangan->status) == 'Sedang Digunakan') status-digunakan
                                        @else status-maintenance
                                        @endif" id="status-preview">
                                        {{ old('status', $ruangan->status) }}
                                    </span>
                                </small>
                            </div>
                        </div>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-warning">
                        <i class="fas fa-save"></i> Perbarui Ruangan
                    </button>
                </div>
            </form>
        </div>
        ...
        <script>
            ...
            statusSelect.addEventListener('change', function() {
                ...
                statusPreview.classList.remove('status-tersedia', 'status-digunakan', 'status-maintenance');
                
                // Add appropriate class based on status
                if (status === 'Tersedia') {
                    statusPreview.classList.add('status-tersedia');
                } else if (status === 'Sedang Digunakan') {
                    statusPreview.classList.add('status-digunakan');
                } else {
                    statusPreview.classList.add('status-maintenance');
                }
            });
